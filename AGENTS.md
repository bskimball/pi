...
- `/ui` selects one of three presentations: `pi` (stock Pi), `apex`, and `claude`. `claude` is a **skin over Apex**, not a separate UI extension: it keeps `PI_APEX_UI=1` and sets `PI_UI_SKIN=claude`, which swaps the receipt glyph set (`●` root, `⎿` continuation) and the composer prompt, paired with the `claude-dark` theme. `PI_APEX_UI=0` applies only to `pi`. Skin glyphs live in `apex/internal/presentation/skin.ts` and are read dynamically at call time, so `/ui` switches take effect without a restart; an unset or unrecognized `PI_UI_SKIN` falls back to the apex glyphs.
