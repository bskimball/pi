...
- Live async workers share that same above-editor dock as an Agents tab (`alt+a` / `/agents`; `alt+t` / `/todos` still collapse). Fusion's lone sidekick instead gets one persistent inline line on the Todos pane, since there is no second pane worth switching to. Triggers and chrome-off behavior: [`CONTEXT.md` § Todo dock](CONTEXT.md#todo-dock).
