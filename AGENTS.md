...
- `/mode work` is operations-first and uses its own prompt rather than the coding-first `agent/SYSTEM.md`. The lead works inline-first and dispatches a closed four-specialist crew (strategist, researcher, author, clerk) — no persistent sidekick, no other agents. Business workflows and integrations remain owned by their project.
