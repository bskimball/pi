...
- Headless by design (execute, not chrome): `bg-process`, `powershell`, `mcp-adapter`, `web-search`, `continual-memory`, `read-guard`, `lsp`, `graphify`. Apex attaches receipt chrome to several of these, skipped entirely when `PI_APEX_UI=0`. `at-path-complete` is also headless: it only wraps scoped `@` autocomplete. Package-loaded `pi-better-edit` owns `read`/`edit` execute; Apex skins those receipts the same way (`better-edit-receipt.ts`, `overrideOwned`).
