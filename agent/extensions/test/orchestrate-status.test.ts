...
describe("strict orchestrator prompt", () => {
  it("injects only while enabled and carries bounded delegation semantics", async () => {
    ...
    assert.match(active.systemPrompt, /Scout discovery, Stevedore integrated checks/);
    assert.match(active.systemPrompt, /Inspector and Stevedore verification-only run concurrently when both apply/);
    assert.match(active.systemPrompt, /Oracle reviews the verified final diff/);
    assert.match(active.systemPrompt, /visible parallel `task` calls/);
    assert.match(active.systemPrompt, /same assistant turn/);
    assert.match(active.systemPrompt, /dependent slices visibly only after prerequisite reports return/);
    assert.match(active.systemPrompt, /Never use an aggregate scheduler such as `task_chain`/);
    assert.match(active.systemPrompt, /If a proposed writer prompt names 3\+ components\/packages/);
    assert.match(active.systemPrompt, /Never send a broad "implement the stage\/feature" assignment/);
    assert.match(active.systemPrompt, /writer prompt generations are limited to one corrective generation/);
    assert.match(active.systemPrompt, /Keep every delegated task visible in the thread/);
    assert.match(active.systemPrompt, /independent slices actually overlapped/);
    assert.doesNotMatch(active.systemPrompt, /use `mission`/);
    ...
    assert.match(active.systemPrompt, /through Stevedore verification-only mode/);
