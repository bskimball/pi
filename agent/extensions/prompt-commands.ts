...
- Enforce an acceptance-sized decomposition before dispatch. A writer work order must own one cohesive outcome and one component/package boundary, explicit files, an observable acceptance condition, and the cheapest local check. If a proposed writer prompt names 3+ components/packages, 3+ ordered units, or combines discovery, architecture, implementation, integration, and broad verification, split it first. Never send a broad "implement the stage/feature" assignment to one writer and decompose reactively through follow-up generations.
- Dispatch independent one-shot slices as separate visible parallel \`task\` calls in the same assistant turn. For persistent or steerable slices, issue separate visible \`task_start\` calls, then one \`task_wait\` per worker after useful lead work. Dispatch dependent slices visibly only after prerequisite reports return. Parallel writers require isolated worktrees plus scoped patch integration. Serialize only real dependencies or overlapping ownership. Never use an aggregate scheduler such as \`task_chain\` in strict orchestrator mode. Use \`task_start\` only for one cohesive slice when steering is genuinely likely; writer prompt generations are limited to one corrective generation, then collect/close and dispatch a narrower fresh slice.
- Use synchronous \`task\` for one-shot barriers whose result is needed before proceeding: Scout discovery, Stevedore integrated checks, Inspector live-page proof, and Oracle review. Dispatch independent one-shot calls together in the same parallel tool turn. After writers settle, Inspector and Stevedore verification-only run concurrently when both apply; Oracle reviews the verified final diff after any resulting fixes.
- Treat a writer report that explicitly says the acceptance criteria are partial, incomplete, failed, or blocked as an unsuccessful slice. Do not continue the same broad assignment repeatedly. Preserve valid work, narrow the remaining contract, and respawn once with explicit missing acceptance criteria.
- Keep the lead context to decisions, slice contracts, assignments, bounded reports, blockers, integration, and gate status. Keep every delegated task visible in the thread. Before finalizing, inspect whether independent slices actually overlapped and whether corrective generations indicate poor decomposition.
- Every implementation diff still requires Oracle review. UI behavior still requires one Inspector pass. Combined lint, format check, typecheck, tests, and build run only after writers settle, through Stevedore verification-only mode. Route failures back to the owning writer, then run one combined pass after fixes.
