...
- Give each writer one cohesive outcome, explicit file ownership, an observable acceptance condition, and the cheapest local check. Parallel writers require isolated worktrees and safe patch integration; serialize overlapping or dependent slices. Use \`task_start\` only when persistence, steering, follow-up, or concurrent lead work is valuable.
...
- Keep the lead context to decisions, slice contracts, assignments, bounded reports, blockers, integration, and gate status. Use \`mission\` for an explicit dependency graph when it removes mechanical dispatch/wait/close turns without hiding product decisions.
