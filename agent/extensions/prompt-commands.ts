...
- Every delegated implementation diff gets a fresh-eyes Oracle review of the actual changed files and diff, including UI code. Tiny integration fixes may use the lead's focused review. The implementer's self-review and Inspector's browser verdict never close a delegated code-review unit.
- Verification is yours to own. After all writers have settled, send the combined worktree to one fresh Stevedore verification-only pass for the requested lint, format check, typecheck, tests, or build; inspect its result and route failures back to the owning slice. Run another combined pass only after those fixes settle. Never run integrated gates inside Artisan/Machinist or while writers are active. Route live browser and screenshot checks to Inspector without asking it to inspect source or diagnose code; use Artisan only when verification requires design judgment or implementation changes.
