...
- Prefer \`task_start\`, do independent lead work, then one \`task_wait\` per worker (default 600s; 900s machinist/artisan; 1200s oracle/stevedore/inspector). Do not poll with \`task_status\`/\`task_wait\` loops, and do not use waiting time to duplicate a scout or writer's investigation. Timeout is not a poll cue and does not kill the worker: same-generation re-wait is blocked for 60s unless \`timeoutSec\` is longer than the last wait (do independent lead work, advance another slice, wait a different worker, or pass a longer \`timeoutSec\`). \`task_status\` is for blockers only. Keep returned evidence compact: outcome, files, findings, validation, blockers — not transcripts or full logs.
