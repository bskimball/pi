...
├── hal-art.ts            truecolor HAL bitmaps (ULTRA / WIDE / MID)
...
`/ui hal` shows a crimson orb+wordmark lockup with an `OPERATIONS CONSOLE` label instead of the shark and star field, with the same keyboard navigation. Three truecolor pixel tiers carry the lockup — ULTRA (105 cells), WIDE (71), MID (46) — picked widest-first when 24-bit color is available and the width fits. Without truecolor, or when no pixel tier fits, it falls back to the 6-row × 19-column block-glyph wordmark with the 3-row × 8-column glyph orb above it. Below 20 columns it reduces to plain `HAL`. The passive landing shows the mark alone — no CUSTOM PROMPTS / CUSTOM AGENTS inventory — while the interactive orb and `/observatory` still list the full inventory. This deliberately mirrors the HAL Desktop app: desktop brand crimson `#9e1b32`, the titlebar's 2:1 solid-to-translucent horizontal banding, and the `HalHeroOrb` crimson-core/cyan-instrument language (crimson core, cyan frame — never an eye). No Work source or desktop runtime is imported.
...
- The orb is 3 rows × 8 columns: a solid `brand` core band with a cyan `accent` rim and limbs. It renders only as the non-truecolor fallback, above the glyph wordmark; the pixel tiers draw orb and wordmark as one lockup.
- `hal-art.ts` is generated by `tools/hal-art/emit-ts.py`; do not hand-edit it.
...
Preview with the real HAL theme and the same bounds checks. `COLORTERM` decides
which tier you are looking at: with it the pixel lockup renders, without it the
glyph fallback does, so check both.

```bash
COLORTERM=truecolor PI_UI_SKIN=hal node --experimental-transform-types agent/extensions/apex/observatory/preview.mjs 8 19 20 40 62 80 120 160
