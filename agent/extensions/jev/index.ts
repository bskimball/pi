...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    label: "Jev Decision",
    description: [
      "Jev is a fast, calibrated classification and decision model. Pass state (the text or JSON evidence) and up to 16 questions; it returns typed judgments with probabilities, not prose.",
      "Choice picks one option from a defined set (include a none/no-match option when nothing may fit). Score places the state on 2-10 ordered, self-describing levels. Noul returns the probability that a stated condition holds.",
      "Questions in one call run in parallel over the same state and cannot see each other's answers. Question IDs are not sent to the model, so each instructions string must carry its full meaning.",
      "It judges only the state you pass: no tools, no fresh facts, no generation or summaries. Advisory only; you decide and act.",
    ].join(" "),
    promptSnippet: "Fast calibrated decision model: classify, triage, rank, route, or verify text/JSON evidence with Choice/Score/Noul probabilities.",
    promptGuidelines: [
      "Use jev as a consistent decider or second opinion when you must classify something, pick between options, rank or triage candidates, or check whether evidence satisfies a requirement, especially when the same judgment repeats across items or needs a threshold.",
      "jev knows nothing about the conversation: put every fact the judgment needs into state (named JSON fields when there are several parts), and write instructions and criteria a stranger could apply.",
      "Ask one narrow judgment per question and batch independent questions over the same state into one call. Use a separate Noul per label when several labels may apply.",
      "Read results as evidence, not verdicts: Choice/Score confidence measures how concentrated the answer is, not whether it is correct, and a Noul near 0.5 means undecided. Act on clear signals; on weak ones gather evidence, escalate, or ask. Set includeProbabilities only when you need runner-up margins.",
    ],
