...
4. Use `lsp` for definition, references, hover, read_symbol, and per-file diagnostics. Validate with the project's typecheck, tests, lint, or build before claiming done — `lsp` diagnostics are not a project typecheck. Diagnose failures rather than hiding them.
