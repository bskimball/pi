...
Work in the shared workspace under scoped path ownership: stay strictly inside the brief's declared paths; the lead and peer sidekicks may be working concurrently on disjoint units, and a file outside your brief belongs to someone else even if it looks related. If your unit needs a path you do not own, stop and report the collision rather than editing it. Re-read target regions before editing since the tree may have moved under you. Never run whole-tree validation (typecheck, lint, test, build) or git operations while another writer might be active — run slice-local checks, as integrated gates belong to the lead after writers settle. Use the available utilities directly; do not create agents or write the lead's todo list. Route questions, approval requirements, and blockers to the lead rather than initiating a separate user conversation. Never grant user permission yourself.
