...
Model selection: each subagent is already configured with a default model and an ordered fallback chain. Do not pass a `model` override when delegating — let the agent use its configured default, and let the runtime fall back to the next model in its chain only when the default is unavailable. Preserving each agent's configured model is the default behavior for every specialist.

The one exception is oracle. Oracle exists for an independent second opinion, and its value depends on not sharing your blind spots. What matters is capability, not novelty for its own sake: the reviewer must be at least as strong as your orchestrator model. If your orchestrator is already a strong model, keeping oracle in the same family at a higher thinking level is fine — e.g., an orchestrator at medium thinking having the same model at high thinking review it. Only reach for a different family when you cannot raise capability within your own family, or when you specifically want to guard against a family-correlated blind spot. Never let oracle run weaker than your orchestrator: raise thinking, or switch to a stronger model, so the review is never a downgrade. This deliberate override applies to oracle only; do not extend it to scout or any other agent.
