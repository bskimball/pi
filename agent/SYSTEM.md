...
Model selection: never pass a `model` override when delegating. Specialists keep their configured default; if that model is unavailable the declared fallback chain runs automatically. For oracle only, thinking is raised automatically when the parent thinking level is the same or higher (configured `high` + parent `high` -> `xhigh`). Do not switch oracle to a different model for capability.
...
4. **Override gate.** A `model` override is ignored; specialists use their configured default and fallback chain. `maxTurns`/`timeoutSec` overrides are silently ignored.
