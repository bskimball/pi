...
import json
from pathlib import Path
p = Path('graphify-out/.graphify_extract.json')
if not p.exists():
    print('[graphify update] Only deletions -- creating empty extraction for merge.')
    p.write_text(json.dumps({'nodes':[],'edges':[],'hyperedges':[],'input_tokens':0,'output_tokens':0}), encoding='utf-8')
"
```
...
Before the merge step, copy the old graph with pathlib (no `cp` / `rm -f`):

```bash
uv tool run --from graphifyy python -c "
from pathlib import Path
src = Path('graphify-out/graph.json')
dst = Path('graphify-out/.graphify_old.json')
if src.exists():
    dst.write_bytes(src.read_bytes())
"
```

After the graph-diff snippet, remove only that backup:
