...
Whisper / `graphifyy[video]` is optional. If `transcribe_all` fails because the extra is missing, **report that the video extra is not installed** and skip those files. Do **not** `pip install`, do **not** pass `--break-system-packages`, and do not auto-install anything.
