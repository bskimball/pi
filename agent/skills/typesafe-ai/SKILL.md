...
description: >
  ...
  In this Pi config, also load when designing or calling the existing jev tool
  (Choice/Score/Noul); evaluate in-session with that tool rather than a new client.
---
...
Load [`PI.md`](PI.md) before designing or evaluating questions. In-session
classification uses the existing `jev` tool; do not add a second client.
Product TypeSafe HTTP/SDK work still follows the live docs below.
