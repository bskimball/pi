...
`agent/settings.json` loads `@ff-labs/pi-fff` and `pi-better-edit` as npm packages. `pi-better-edit` supplies hash-addressed `read`/`edit` tools, so Apex does not register its legacy unified edit or builtin read wrapper. The `pi-mcp-adapter` dependency is composed locally via `mcp-adapter.ts` instead of package-loaded. (`pi-sticky-input` was dropped at Pi 0.84.1 in favor of the built-in `tuiMode: "fullscreen"`.)
